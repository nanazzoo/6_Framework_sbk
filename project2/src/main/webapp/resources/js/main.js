...
if (saveId != null) {
  ...
  saveId.addEventListener("change", function () {
    ...
    if (this.checked) {
      ...
      const str =
        "개인 정보 보호를 위해 개인 PC에서의 사용을 권장합니다. " +
...
selectEmail.addEventListener("click", (e) => {
  ...
  $.ajax({
    ...
    success: (member) => {
      ...
      // 2) Jackson 라이브러리 이용 시 JS 객체로 자동 변환
      ...
      if (member == null) {
        ...
        li6.innerText = "탈퇴 여부: " + member.memberDeleteFlag;
...
function selectMemberList() {
  ...
  $.ajax({
    ...
    success: (memberList) => {
      ...
      //
      document.getElementById("count").innerText = memberList.length + "명";
    },
    error: () => {
