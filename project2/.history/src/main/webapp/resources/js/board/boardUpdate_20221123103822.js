...
for (let i = 0; i < inputImage.length; i++) {
  ...
  inputImage[i].addEventListener("change", (event) => {
    ...
    if (event.target.files[0] != undefined) {
      ...
      //지정된 input type="file"의 파일을 읽어와 url값으로 저장
  ...
  // 미리보기 삭제버튼 클릭 시 동작
  deleteImage[i].addEventListener("click", (event) => {
    // 미리보기 이미지가 존재할 때만 삭제
...
function updateValidate() {
  ...
  // form 태그 내부 input 태그로 추가하여 같이 제출하게 만들기

  // Array.from(deleteSet) -> deleteSet을 배열로 변환

  // JS 배열의 특징: 배열을 HTML요소 또는 console에 출력하면 "요소, 요소, 요소" 형태의 문자열로 변환

  document.getElementById("deleteList").value = console.log(
